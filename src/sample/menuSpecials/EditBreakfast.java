...
public class EditBreakfast implements Initializable {
    @FXML
    private TableView<DailySpecialsModelTable> breakfastTable;
    @FXML
    ...
    private void loadTables(){
        connectToDatabase();
        setupTables(colItemType, colItemName, colDescription, breakfastTable, obListItems);
    }
    ...
    private void onEdit() {
        ...
        if (breakfastTable.getSelectionModel().getSelectedItem() != null) {
            DailySpecialsModelTable selectedItem = breakfastTable.getSelectionModel().getSelectedItem();
            dialogBreakfast(selectedItem.getItemType(), selectedItem.getItemName(), selectedItem.getItemDescription(), selectedItem.getItemID(), 1);
            // clear the selection of the table (technically not needed since this is only one table.
            breakfastTable.getSelectionModel().clearSelection();
        }
    }
     ...
     */
    private void dialogBreakfast(String itemType, String itemName, String itemDescription, String itemID, int controller){
        ...
        dropBox.getItems().addAll("Cereal", "Sides");
        if(itemType.equals("")){
        ...
        dialog.setResultConverter(dialogButton -> {
            ...
            if (dialogButton == submitButtonType) {
                if(controller == 0) {
                    DatabaseQueries query = new DatabaseQueries();
                    String mysqlQuery = "INSERT IGNORE INTO breakfast(typeOfItem, itemName, itemDescription) " +
                            "VALUES (?, ?, ?)";
    ...
    private void submitPressed(String itemType, String itemName, String itemDescription, String itemID){
        String mysqlQuery = "UPDATE " + "breakfast " + "SET typeOfItem = ?, itemName = ?, itemDescription = ? WHERE itemID = ?";
        DatabaseQueries query = new DatabaseQueries();
