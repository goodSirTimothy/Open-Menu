...
public class EditMonthlySpecial implements Initializable {
    @FXML
    private TableView<MonthlySpecialsModelTable> monthlyTable;
    @FXML
    private TableColumn<MonthlySpecialsModelTable, String> colItemType, colItemName, colDescription;
    private ObservableList<MonthlySpecialsModelTable> obListItems = FXCollections.observableArrayList();
    ...
    private void loadTables(){
        connectToDatabase();
        setupTables(colItemType, colItemName, colDescription, monthlyTable, obListItems);
    }
    ...
    private void connectToDatabase(){
        ...
        try {
            ...
            ResultSet rs = query.queryToDatabase("SELECT * " +
                    ...
                    ";");
            while(rs.next()){
                obListItems.add(new MonthlySpecialsModelTable(rs.getString("itemID"), rs.getString("typeOfItem"), rs.getString("itemName"), rs.getString("itemDescription")));
            }
            rs.close();
    ...
    private void onEdit() {
        ...
        if (monthlyTable.getSelectionModel().getSelectedItem() != null) {
            MonthlySpecialsModelTable selectedItem = monthlyTable.getSelectionModel().getSelectedItem();
            dialogMonthlySpecial(selectedItem.getItemType(), selectedItem.getItemName(), selectedItem.getItemDescription(), selectedItem.getItemID(), 1);
     ...
     */
    private void dialogMonthlySpecial(String itemType, String itemName, String itemDescription, String itemID, int controller){
    ...
    private void submitPressed(String itemType, String itemName, String itemDescription, String itemID){
        String mysqlQuery = "UPDATE " + "monthlySpecial " + "SET typeOfItem = ?, itemName = ?, itemDescription = ? WHERE itemID = ?";
        DatabaseQueries query = new DatabaseQueries();
